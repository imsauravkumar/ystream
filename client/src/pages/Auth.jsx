import { Eye, EyeOff, LogIn, Mail, ShieldCheck, Sparkles, UserPlus, UserRound, UsersRound } from "lucide-react";
    ...
    <main className="min-h-screen bg-ink px-4 py-5 text-zinc-50 sm:px-6 lg:px-10">
      ...
      <section className="mx-auto grid min-h-[calc(100vh-6rem)] max-w-6xl items-center gap-6 py-8 lg:grid-cols-[minmax(0,1fr)_27rem] lg:gap-8">
        <div className="space-y-5 text-center lg:text-left">
          <p className="mx-auto inline-flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950/80 px-3 py-1 text-sm font-semibold text-brand lg:mx-0">
            <Sparkles size={15} />
            Watch parties without drift
          </p>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-zinc-800 bg-panel/80 p-4 text-left">
              <UsersRound className="mb-3 text-brand" size={20} />
              <p className="font-bold text-white">Private synced rooms</p>
              <p className="mt-1 text-sm leading-5 text-muted">Invite friends with a single room code.</p>
            </div>
            <div className="rounded-lg border border-zinc-800 bg-panel/80 p-4 text-left">
              <ShieldCheck className="mb-3 text-brand" size={20} />
              <p className="font-bold text-white">Host-first controls</p>
              <p className="mt-1 text-sm leading-5 text-muted">Grant playback access when the room is ready.</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-panel p-4 shadow-glow sm:p-6">
          <div className="mb-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Welcome back</p>
            <h2 className="mt-1 text-2xl font-black">{mode === "login" ? "Login to Ystream" : "Create your account"}</h2>
          </div>
          <div className="mb-5 grid grid-cols-2 gap-2 rounded-lg bg-zinc-950 p-1">
            <button
              className={`h-10 rounded-md text-sm font-bold transition ${
                mode === "login"
                  ? "bg-brand text-zinc-950 shadow-[0_8px_22px_rgba(56,189,248,0.16)]"
