      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 2xl:grid-cols-3">
        {loading &&
          Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950/70">
        ...
        {!loading && results.length === 0 && (
          <div className="rounded-lg border border-dashed border-zinc-800 bg-zinc-950/60 p-5 text-sm leading-6 text-muted sm:col-span-2 2xl:col-span-3">
            Search for a song, live set, or YouTube URL to start filling the shared queue.
          </div>
        )}
        {results.map((video) => (
