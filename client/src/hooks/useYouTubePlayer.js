...
export function useYouTubePlayer({ onPlay, onPause, onEnded, onReady }) {
  ...
  const load = useCallback((nextVideoId, seconds = 0, autoplay = false) => {
  ...
  const isMuted = useCallback(() => Boolean(playerRef.current?.isMuted?.()), []);
  ...
  const getState = useCallback(() => playerRef.current?.getPlayerState?.(), []);
